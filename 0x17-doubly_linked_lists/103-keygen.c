...
int main(int argc, char *argv[])
{
	...
	map = "A-CHRDw87lNS0E9B2TibgpnMVys5XzvtOGJcYLU+4mjW6fxqZeF3Qa1rPhdKIouk";
	strcpy(key, "      ");
	...
	key[0] = map[(username_len ^ 59) & 63];

	char_sum = 0;
	for (i = 0; i < username_len; i++)
		char_sum += argv[1][i];
	key[1] = map[(char_sum ^ 79) & 63];

	product = 1;
	for (i = 0; i < username_len; i++)
		product *= argv[1][i];
	key[2] = map[(product ^ 85) & 63];
	...
	key[4] = map[(product ^ 239) & 63];

	random_value = 0;
	for (i = 0; i < (unsigned int)argv[1][0]; i++)
		random_value = rand();
	key[5] = map[(random_value ^ 229) & 63];
